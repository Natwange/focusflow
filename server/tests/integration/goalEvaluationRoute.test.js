...
describe("GET /goals/:id/evaluation", () => {
  beforeEach(async () => {
    jest.clearAllMocks();
    db.users.clear();
    db.goals.clear();
  ...
  test("requires auth", async () => {
    const app = createApp();
    const res = await request(app).get("/goals/goal_1/evaluation");
    expect(res.status).toBe(401);
  });

  test("returns goal evaluation for owner", async () => {
    const app = createApp();
    ...
    const res = await agent.get("/goals/goal_1/evaluation");
