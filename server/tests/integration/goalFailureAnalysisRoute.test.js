...
describe("GET /goals/:id/failure-analysis", () => {
  beforeEach(async () => {
    jest.clearAllMocks();
    db.users.clear();
    db.goals.clear();
  ...
  test("requires auth", async () => {
    const app = createApp();
    const res = await request(app).get("/goals/goal_1/failure-analysis");
  ...
  test("returns 403 for non-owner", async () => {
    ...
    const res = await agent.get("/goals/goal_1/failure-analysis");
    expect(res.status).toBe(403);
    expect(res.body.error).toBe("Forbidden: goal does not belong to this user");
  });

  test("returns failure analysis for owner", async () => {
    const app = createApp();
    const agent = request.agent(app);
    ...
    expect(Array.isArray(res.body.failureAnalysis.failureModes)).toBe(true);
    expect(res.body.failureAnalysis).toHaveProperty("primaryFailureMode");
    expect(res.body.failureAnalysis).toHaveProperty("explanation");
    expect(res.body.failureAnalysis).toHaveProperty("severity");
  });
});
