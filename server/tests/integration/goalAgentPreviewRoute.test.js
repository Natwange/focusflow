...
describe("GET /goals/:id/agent-preview", () => {
  beforeEach(async () => {
    jest.clearAllMocks();
    db.users.clear();
    db.goals.clear();
  ...
  test("requires auth", async () => {
    const app = createApp();
    const res = await request(app).get("/goals/goal_1/agent-preview");
  ...
  test("returns orchestrated goal agent output for owner", async () => {
    const app = createApp();
    const agent = request.agent(app);
    const loginRes = await loginAs(agent, {
      email: "alice@example.com",
    ...
    expect(typeof res.body.agentRunId).toBe("string");
