...
- If canRebalance is true, explain what is wrong and ask for approval before apply_goal_rebalance. Rebalance previews preserve learning order: earlier sections (lower unitStart) are never scheduled after later sections just to free capacity. If order cannot fit before the deadline, the preview recommends extend_deadline — suggest preview_goal_adjustment / deadline extension instead of a plan that skips ahead. When get_agent_strategy_memory hasEnoughData and rebalance successRate is strong, you may note that past accepted rebalances helped — still require user confirmation.
